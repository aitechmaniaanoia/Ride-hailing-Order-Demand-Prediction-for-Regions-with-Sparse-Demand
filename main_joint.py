...
trainX = lstms
testX = np.ones( (N_TEST_EXAMPLES, joint_model.seq_len, joint_model.feature_len) )
...
trainY = outs
testY  = np.ones( (N_TEST_EXAMPLES, joint_model.feature_len) )
...
trainClass = np.reshape(np.minimum( np.ones_like(outs), outs ), (-1,))
from sklearn.utils import class_weight
class_weights = class_weight.compute_class_weight('balanced',
        list(np.unique(trainClass)),
        list(trainClass))

print(class_weights)

joint_model.build_model(trainX,trainY,
        testX, testY,
        trainimg, testimg,
        traintopo, testtopo,
