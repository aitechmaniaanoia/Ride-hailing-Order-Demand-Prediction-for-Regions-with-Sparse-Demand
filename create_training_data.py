# -*- coding: utf-8 -*-
"""
Created on Mon Nov 11 16:51:27 2019
        ...
        for hour in range(24):
            for minute_bin in [0,1]:
                ...
                for x in range(0,50):
                    for y in range(0,30):
                        try:
                            heatmap[y][x] = demand_groups.get_group( (month, day, hour, minute_bin, x, y) ).size/7 # counts 1 per column, so 7 per order
                ...
                maps.append(heatmap)
maps = np.array(maps)
np.savez_compressed("demand.npz", maps)
