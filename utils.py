...
def log_error(y_true, y_pred):
    return K.mean(K.square(K.log(1+y_pred) - K.log(1+y_true)))
...
def mean_absolute_percentage_error_revise(y_true, y_pred):
    ...
    mean_lable_float32 = np.float32(model.mean_label)#.astype(np.float32)
    return 10. * K.mean(diff, axis=-1) + model.loss_lambda / K.square(mean_lable_float32) * K.mean(K.square(y_pred - y_true), axis=-1)
